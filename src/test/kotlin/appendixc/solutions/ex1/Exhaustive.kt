...
object Prop {
  ...
  val p4 =
    forAllPar(Prop.pint)(n => equal(Par.map(n)(y => y), n))
...
object Gen {
  ...
  */
  ...
  val maxProp = forAll(listOf(Gen.smallInt)) { l =>
    val max = l.max
  ...
  val maxProp1 = forAll(listOf1(Gen.smallInt)) { l =>
    val max = l.max
    !l.exists(_ > max) // No value greater than `max` should exist in `l`
  }

  val sortedProp = forAll(listOf(Gen.smallInt)) { l =>
    val ls = l.sorted
