package appendixc.solutions.ex1
...
data class Prop(val check: (MaxSize, TestCases, RNG) -> Result) {
    fun and(p: Prop): Prop = Prop { max, n, rng ->
        when (val prop = check(max, n, rng)) {
            ...
            is Proved -> p.check(max, n, rng)
            else -> prop
        }
    }

    fun or(p: Prop): Prop = Prop { max, n, rng ->
        when (val prop = check(max, n, rng)) {
            // In case of failure, run the other prop.
            is Falsified -> p.tag(prop.failure).check(max, n, rng)
            else -> prop
        }
    }
     ...
     */
    fun tag(msg: String) = Prop { max, n, rng ->
        when (val prop = check(max, n, rng)) {
            is Falsified -> Falsified(msg + "\n" + prop.failure, prop.successes)
            else -> prop
        }
    }
    ...
    companion object {
        ...
        fun <A> randomStream(g: Gen<A>)(rng: RNG): Stream<A> =
        Stream.unfold(rng)(rng -> Some(g.sample.run(rng)))

        fun <A> forAll(as: Gen<A>)(f: (A) -> Boolean): Prop = Prop {
...
data class Gen<out A>(sample: State<RNG, A>) {
