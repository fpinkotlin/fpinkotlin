...
class Exercise_6_1 : WordSpec({
    ...
    "nonNegativeInt" should {
        ...
        "!return Int.MAX_VALUE when nextInt() yields Int.MAX_VALUE" {
            ...
            nonNegativeInt(rngMax) shouldBe Pair(Int.MAX_VALUE, unusedRng)
        }

        "!return Int.MAX_VALUE when nextInt() yields Int.MIN_VALUE" {

            val rngMin = object : RNG {
                override fun nextInt(): Pair<Int, RNG> = Pair(Int.MIN_VALUE, unusedRng)
            }

            nonNegativeInt(rngMin) shouldBe Pair(Int.MAX_VALUE, unusedRng)
        }

        "!return 0 when nextInt() yields -1" {

            val rngNeg = object : RNG {
                override fun nextInt(): Pair<Int, RNG> = Pair(-1, unusedRng)
            }
