...
class Solution_5_7 : WordSpec({
    ...
    fun <A, B> Stream<A>.map(f: (A) -> B): Stream<B> =
            this.foldRight(thnk(empty<B>()), { h, t -> cons(thnk(f(h)), t) })
    //end::map[]

    //tag::filter[]
    fun <A> Stream<A>.filter(f: (A) -> Boolean): Stream<A> =
            this.foldRight(thnk(empty<A>()), { h, t ->
                if (f(h)) cons(thnk(h), t)
                else t()
            })
    //end::filter[]
    ...
    fun <A> Stream<A>.append(sa: () -> Stream<A>): Stream<A> =
            foldRight(sa, { h, t -> cons(thnk(h), t) })
    //end::append[]

    //tag::flatmap[]
    fun <A, B> Stream<A>.flatMap(f: (A) -> Stream<B>): Stream<B> =
            foldRight(thnk(empty<B>()), { h, t -> f(h).append(t) })
    //end::flatmap[]

    "Stream.map" should {
        "apply a function to each evaluated element in a stream" {
            val s = Stream.of(1, 2, 3, 4, 5)
            s.map { (it * 2).toString() }.toList() shouldBe List.of("2", "4", "6", "8", "10")
        }
    ...
    "Stream.filter" should {
        "return all elements of a stream that conform to a predicate" {
            val s = Stream.of(1, 2, 3, 4, 5)
            s.filter { it % 2 == 0 }.toList() shouldBe List.of(2, 4)
        }
        "return no elements of an empty stream" {
            empty<Int>().filter { it % 2 == 0 }.toList() shouldBe List.empty()
        }
    }
    "Stream.append" should {
        "append two streams to each other" {
            ...
            s1.append { s2 }.toList() shouldBe List.of(1, 2, 3, 4, 5, 6)
        }
    ...
    "Stream.flatMap" should {
        "apply a function that can fail to each evaluated element in a stream" {
            val s = Stream.of(1, 2, 3, 4, 5)
            s.flatMap { Stream.of("$it", "${it * 2}") }.toList() shouldBe
                    List.of("1", "2", "2", "4", "3", "6", "4", "8", "5", "10")
        }
    }
})